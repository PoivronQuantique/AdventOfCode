...
namespace AdventOfCode.Jours
{
    public class Jour_10 : Jour_abs
    {
        ...
        private void InitialisationLigne(string ligne, int NumeroLigne)
        {
            ...
            {
                Grille.Add(new Tile(j, NumeroLigne, ligne.Substring(j,1)));
            }
        }
        #endregion
        ...
        private long ProcessPart1()
        {
            ...
            return Grille.CalculateLoop() / 2;
        }
        private long ProcessPart2()
        {
            var Loop = Grille.Where(t => t.IsLoop);
            if(Debug)
                Console.WriteLine(Grille.ToString());
            return Grille.Where(t => !t.IsLoop).Count(t => t.EstADroite);
        }
        ...
        private class Tile
        {
            private static Dictionary<char, List<Direction>> Encodage { get; set; } = new Dictionary<char, List<Direction>>()
            {
                { '|', new List<Direction>() { Direction.Top, Direction.Bottom} },
                { '-', new List<Direction>() { Direction.Right, Direction.Left } },
                { 'L', new List<Direction>() { Direction.Top, Direction.Right } },
                { 'J', new List<Direction>() { Direction.Top, Direction.Left } },
                { '7', new List<Direction>() { Direction.Left, Direction.Bottom } },
                { 'F', new List<Direction>() { Direction.Right, Direction.Bottom } },
                { '.', new List<Direction>() { } },
                { 'S', new List<Direction>() { Direction.Top, Direction.Right, Direction.Left, Direction.Bottom } }    
            };
            public int X { get; set; }
            public int Y { get; set; }
            public string Cle { get; private set; }
            public bool IsLoop { get; set; }
            public bool EstAGauche { get; set; } = false;
            public bool EstADroite { get; set; } = false;

            public string direction { get; set; }
            public List<Direction> Directions { get; set; }
            public List<Tile> NextTiles { get; set; } = new List<Tile>();
            public Dictionary<Direction, Tile> Voisins = new Dictionary<Direction, Tile>();

            public Tile(int x, int y, string direction)
            {
                this.X = x;
                this.Y = y;
                this.Cle = $"{x}-{y}";
                this.direction = direction;
                Directions = Tile.Encodage[direction[0]].ToList();
            }
        }
        private class Grid : List<Tile>
        {
            public Tile StartTile { get; set; }
            private Tile[,] Tiles = new Tile[140, 140];
            ...
            public new void Add(Tile t)
            {
                base.Add(t);
                Tiles[t.X, t.Y] = t;
                if(t.Directions.Count == 4)
                    this.StartTile = t;
            }

            public long CalculateLoop()
            {
                foreach(Tile t in this)
                {
                    foreach(Direction d in t.Directions)
                    {
                        int offsetX = 0;
                        int offsetY = 0;
                        Direction? DirectionAttendue = null;
                        switch (d)
                        {
                            case Direction.Left:
                                DirectionAttendue = Direction.Right;
                                offsetX = -1;
                                break;
                            case Direction.Right:
                                DirectionAttendue = Direction.Left;
                                offsetX = 1;
                                break;
                            case Direction.Top:
                                DirectionAttendue = Direction.Bottom;
                                offsetY = -1;
                                break;
                            case Direction.Bottom:
                                DirectionAttendue = Direction.Top;
                                offsetY = 1;
                                break;
                        }
                        if (DirectionAttendue.HasValue && t.X + offsetX >= 0 && t.X + offsetX < 140 && t.Y + offsetY >= 0 && t.Y + offsetY < 140)
                        {
                            if (Tiles[t.X + offsetX, t.Y + offsetY].Directions.Contains(DirectionAttendue.Value))
                            {
                                t.NextTiles.Add(Tiles[t.X + offsetX, t.Y + offsetY]);
                                t.Voisins.Add(d, Tiles[t.X + offsetX, t.Y + offsetY]);
                            }
                        }
                    }
                }
                StartTile.direction = "L";
                MarquageDroiteGauche();
                return ItemsLoop();
            }
            private void MarquageDroiteGauche()
            {
                Tile curr = StartTile;
                Tile prec = null;
                Dictionary<string, bool> marquage = new Dictionary<string, bool>();
                long nb = 1;
                do
                {
                    marquage.Add(curr.Cle, true);
                    var next = curr.Voisins.First(v => v.Value != prec);
                    MarquerTuilesAdjacentes(next.Value, next.Key);
                    prec = curr;
                    curr = next.Value;
                    nb++;
                }
                while (!marquage.TryGetValue(curr.Cle, out bool temp));

                for(int i = 0; i < 140; i++)
                {
                    bool droiteOuverte = false;
                    for(int j = 0; j < 140; j++)
                    {
                        if(droiteOuverte)
                        {
                            if (Tiles[j, i].EstAGauche || Tiles[j, i].IsLoop)
                            {
                                droiteOuverte = false;
                            }
                            else
                            {
                                    Tiles[j, i].EstADroite = true;
                            }
                        }
                        else
                        {
                            if (Tiles[j, i].EstADroite)
                            {
                                droiteOuverte = true;
                            }
                        }
            ...
            public void MarquerTuilesAdjacentes(Tile t, Direction d)
            {
                List<Tuple<int, int>>offsetsGauches = new List<Tuple<int, int>>();
                List<Tuple<int, int>>offsetsDroits = new List<Tuple<int, int>>();
                switch(d)
                {
                    case Direction.Top:
                        switch(t.direction[0])
                        {
                            case '|':
                                offsetsGauches.Add(new Tuple<int, int>(-1, 0));
                                offsetsDroits.Add(new Tuple<int, int>(1, 0));
                                break;
                            case '7':

                                offsetsDroits.Add(new Tuple<int, int>(1, 0));
                                offsetsDroits.Add(new Tuple<int, int>(1, -1));
                                offsetsDroits.Add(new Tuple<int, int>(0, -1));
                                break;
                            case 'F':
                                offsetsGauches.Add(new Tuple<int, int>(-1, 0));
                                offsetsGauches.Add(new Tuple<int, int>(-1, -1));
                                offsetsGauches.Add(new Tuple<int, int>(0, -1));
                                break;
                        }
                        break;
                    case Direction.Right:
                ...
                foreach (var tup in offsetsGauches)
                {
                    if(t.X + tup.Item1 >= 0 && t.X + tup.Item1 < 140 && t.Y + tup.Item2 >= 0 && t.Y + tup.Item2 < 140)
                        this.Tiles[t.X + tup.Item1,t.Y + tup.Item2].EstAGauche = true;
                }
                foreach (var tup in offsetsDroits)
                {
                    if (t.X + tup.Item1 >= 0 && t.X + tup.Item1 < 140 && t.Y + tup.Item2 >= 0 && t.Y + tup.Item2 < 140)
                        this.Tiles[t.X + tup.Item1,t.Y + tup.Item2].EstADroite = true;
                }
            }
            public long ItemsLoop()
            {
                Dictionary<string, bool> marquage = new Dictionary<string, bool>();
                List<Tile> items = new List<Tile>() { StartTile };
                while(items.Count > 0)
                {
                    Tile t = items[0];
                    t.IsLoop = true;
                    marquage.Add(t.Cle, true);
                    items.AddRange(t.NextTiles.Where(nt => !marquage.TryGetValue(nt.Cle, out bool temp)));
                    items.RemoveAll(nt => marquage.TryGetValue(nt.Cle, out bool temp));
                }
                return marquage.Count;
            }

            public override string ToString()
            {
                List<string> lines = new List<string>();
                int xMax = this.Max(t => t.X);
                int yMax = this.Max(t => t.Y);
                for(int i = 0; i <= yMax; i++)
                {
                    string line = "";
                    for(int j = 0; j <= xMax; j++)
                    {
                        Tile tile = this.First(t => t.Y == i && t.X == j);
                        line += tile.IsLoop ? tile.direction : tile.EstAGauche? "G" : tile.EstADroite ? "D" : ".";
                    }
                    lines.Add(line);
                }
                return string.Join("\r\n", lines);
