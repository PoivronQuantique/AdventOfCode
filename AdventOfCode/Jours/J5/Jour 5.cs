...
namespace AdventOfCode.Jours.J5
{
    public class Jour_5
    {
        ...
        private List<double> Graines { get; set; } = new List<double>();
        private Dictionary<Tuple<double, double>, double> Sol { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> Fertilizer { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> Water { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> Light { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> Temperature { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> Humidite { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> Emplacement { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesSol { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesFertilizer { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesWater { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesLight { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesTemperature { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesHumidite { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private Dictionary<Tuple<double, double>, double> PlagesEmplacement { get; set; } = new Dictionary<Tuple<double, double>, double>();
        private List<Dictionary<Tuple<double, double>, double>> DataMap{ get; set; } = new List<Dictionary<Tuple<double, double>, double>>();
        private List<Dictionary<Tuple<double, double>, double>> PlagesDataMap{ get; set; } = new List<Dictionary<Tuple<double, double>, double>>();
        #endregion

        #region Constructeur
        public Jour_5(bool debug = false)
        {
            #region Input Data
            ...
            DataMap.Add(Sol          );
            DataMap.Add(Fertilizer   );
            DataMap.Add(Water        );
            DataMap.Add(Light        );
            DataMap.Add(Temperature  );
            DataMap.Add(Humidite     );
            DataMap.Add(Emplacement);
            PlagesDataMap.Add(PlagesSol          );
            PlagesDataMap.Add(PlagesFertilizer   );
            PlagesDataMap.Add(PlagesWater        );
            PlagesDataMap.Add(PlagesLight        );
            PlagesDataMap.Add(PlagesTemperature  );
            PlagesDataMap.Add(PlagesHumidite     );
            PlagesDataMap.Add(PlagesEmplacement);
            int i = 0;
        ...
        {
            Graines = Line.Split(':')[1].Trim().Split(' ').Select(double.Parse).ToList();
        }
        /// <summary>
        ...
        private void InitialisationLigne(int Numero, string ligne)
        {
            ...
            DataMap[Numero].Add(new Tuple<double, double>(PremierSplit[0], PremierSplit[0] + PremierSplit[2] - 1), PremierSplit[1]);
        }

        private void InitialisationPlages()
        {
            for(int idm = 0; idm < DataMap.Count; idm++)
            {
                var dm = DataMap[idm];
                List<double> bornes = dm.Keys.Select(k => k.Item1).Union(dm.Keys.Select(k => k.Item2)).OrderBy(l=>l).ToList();
                for(int i = 0; i < bornes.Count - 1; i++)
                {
                    var data = dm.Keys.Where(k => k.Item1 <= bornes[i] && k.Item2 <= bornes[i]).Select(k => dm[k] - k.Item1).ToList();
                    //PlagesDataMap[idm].Add(new Tuple<double, double>, )
                }
            }
        }
        #endregion
        ...
        public double Partie1()
        {
            int NbLignes = Lines.Count;
            double TotalPart1 = 0;

            //foreach (long g in Graines)
            //    TotalPart1 += EmplacementsGraines(g);
            TotalPart1 += EmplacementsGraines(Graines[0]);
        ...
        #region Process
        private double EmplacementsGraines(double Graine)
        {
            List<double> tempMap = new List<double>() {  Graine };
            for(int i = 0; i < 7; i++)
            { 
                List<double> tempMap2 = new List<double>();
                foreach (double r in tempMap)
                {
                    tempMap2.AddRange(MappingN(i, r));
                }
                tempMap.Clear();
                tempMap.AddRange(tempMap2.Distinct());
                tempMap2.Clear();
            }
            Console.WriteLine(String.Join("\r\n", tempMap) + "\r\n");
            return tempMap.Min(l => l);
        }

        List<double> MappingN(int indiceMap, double nomeroN)
        {
            List<double> Mapping = new List<double>();
            var resMap = DataMap[indiceMap].Keys.Where(k => k.Item1 <= nomeroN && k.Item2 <= nomeroN).ToList();
            if (resMap.Count == 0)
            {
                Mapping.Add(nomeroN);
            }
            ...
            {
                foreach(var k in resMap)
                {
                    double offset = nomeroN - k.Item1;
                    Mapping.Add(DataMap[indiceMap][k] + offset);
                }
            }
            return Mapping;
        }
