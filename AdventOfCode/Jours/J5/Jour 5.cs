...
namespace AdventOfCode.Jours.J5
{
    public class Jour_5
    {
        ...
        private Dictionary<Tuple<long, long>, long> Sol { get; set; }           = new Dictionary<Tuple<long, long>, long>();
        private Dictionary<Tuple<long, long>, long> Fertilizer { get; set; }    = new Dictionary<Tuple<long, long>, long>();
        private Dictionary<Tuple<long, long>, long> Water { get; set; }         = new Dictionary<Tuple<long, long>, long>();
        private Dictionary<Tuple<long, long>, long> Light { get; set; }         = new Dictionary<Tuple<long, long>, long>();
        private Dictionary<Tuple<long, long>, long> Temperature { get; set; }   = new Dictionary<Tuple<long, long>, long>();
        private Dictionary<Tuple<long, long>, long> Humidite { get; set; }      = new Dictionary<Tuple<long, long>, long>();
        private Dictionary<Tuple<long, long>, long> Emplacement { get; set; }   = new Dictionary<Tuple<long, long>, long>();
        private List<Dictionary<Tuple<long, long>, long>> DataMap{ get; set; }  = new List<Dictionary<Tuple<long, long>, long>>();
        #endregion
        ...
        public double Partie1()
        {
            ...
            foreach (long g in Graines)
                TotalPart1 = Math.Min(TotalPart1, EmplacementsGraines(g));
        ...
        #region Process
        private long EmplacementsGraines(long Graine)
        {
            List<long> tempMap = new List<long>() { Graine };
            for (int i = 0; i < DataMap.Count; i++)
            {

                List<long> tempMap2 = new List<long>();
                foreach (long r in tempMap)
                {
        ...
        List<long> MappingN(int indiceMap, long numeroN)
        {
            List<long> Mapping = new List<long>();
            var resMap = DataMap[indiceMap].Keys.Where(k => k.Item1 <= numeroN && k.Item2 >= numeroN).ToList();
            if (resMap.Count == 0)
            {
                Mapping.Add(numeroN);
            }
            else
            {
                ...
                {
                    Mapping.Add(numeroN + DataMap[indiceMap][k]);
                }
            }
            return Mapping;
        }
        ...
        {
            int num = 0;
            foreach(var dm in DataMap)
            {
                var keys = dm.Keys.OrderBy(m => m.Item1).ToList();
                for(int i = 0; i < keys.Count - 1; i++)
                {
                    if (keys[i].Item2 >= keys[i+1].Item1)
                    {
                        Console.WriteLine(num + " - " + i + " : " + keys[i].Item2 + " / " + keys[i + 1].Item1);
                    }
                }
                num++;
            }
