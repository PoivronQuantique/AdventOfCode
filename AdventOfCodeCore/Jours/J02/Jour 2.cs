...
namespace AdventOfCodeCore.Jours
{
    public class Jour_2 : Jour_abs
    {
        ...
        {
            return Games.Where(g=>g.IsGamePossibleWith(12,13,14)).Sum(g=>g.Id);
        }
        private long ProcessPart2()
        {
            return Games.Sum(g=>g.getMinimumSetPower());
        }
        #endregion
        ...
        private class Game : List<Subset>
        {
            public long Id { get; private set; }
            public Game(string inputLine)
            {
                var Split = inputLine.Split(':').ToList();
                this.Id = long.Parse(Split.First().Split(' ').Last().Trim());
                var subsets = Split[1].Split(';').Select(s=>s.Trim()).ToList();
                foreach(var sub in subsets)
                {
                    var cubes = sub.Split(',').Select(c=>c.Trim());
                    Subset subset = new Subset();
                    foreach (var c in cubes)
                    {
                        var c1 = c.Split(' ').ToList();
                        long value = long.Parse(c1.First());
                        switch(c1.Last())
                        {
                            case "red":
                                subset.Reds = value;
            ...
            public long getMinimumSetPower()
            {
                return this.Max(s => s.Reds) * this.Max(s => s.Greens) * this.Max(s => s.Blues);
            }
            public bool IsGamePossibleWith(long Reds, long Greens, long Blues)
            {
                return !this.Exists(s => s.Reds > Reds || s.Greens > Greens || s.Blues > Blues);
            }
        }

        private class Subset
        {
            public long Reds { get; set; } = 0;
            public long Greens { get; set; } = 0;
            public long Blues { get; set; } = 0;
        }
        #endregion
    }
}
